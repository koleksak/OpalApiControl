...
def run_model(project=None, model=None, raw=None, dyr=None, xls=None, path=None):
    ...
    while True:
        ...
        if t == -1:  # end of data acquisition
            ...
            if(streaming.check_events(sim.simulationTime,sim.t_acq)):
                streaming.eventQueue, streaming.eventTimes, streaming.eventItems = sim.send_event_signals(streaming.eventQueue)
